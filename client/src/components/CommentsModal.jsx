    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-xl shadow-2xl p-5 border border-gray-100 dark:border-slate-800 relative">
        ...
        <div className="space-y-4 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
          ...
          {comments.map((c) => (
            <div key={c._id} className="border-b border-gray-50 dark:border-slate-800/50 pb-3 flex justify-between items-start group">
              <div className="flex gap-3">
               ...
               {/* Delete button logic */}
               {(userId === c.user?._id || userId === postOwnerId) && (
                   <button 
                        onClick={() => setDeletingCommentId(c._id)}
                        className="text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition p-1.5 rounded-md hover:bg-red-50 dark:hover:bg-red-900/20 cursor-pointer"
                   >
