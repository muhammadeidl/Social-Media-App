              ...
              rendered.push(
                ...
                >
                  ...
                  >
                    ...
                    {message.message_type === "post" && message.shared_post && (
                      <div 
                        className={`w-64 sm:w-72 rounded-xl overflow-hidden cursor-pointer transition-all border shadow-sm my-1
                           ${isCurrentUser 
                             ? "bg-white/10 border-white/20 backdrop-blur-md" 
                             : "bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700"
                           }`}
                        onClick={() => navigate(`/profile/${message.shared_post.user?._id}?postId=${message.shared_post._id}`)} 
                      >
                         {/* Header */}
                         <div className={`flex items-center gap-2 p-3 pb-2 border-b ${isCurrentUser ? "border-white/10" : "border-gray-100 dark:border-slate-700"}`}>
                             <img src={message.shared_post.user?.profile_picture} className="w-6 h-6 rounded-full object-cover" alt="" />
                             <span className={`font-semibold text-[13px] ${isCurrentUser ? "text-white" : "text-gray-900 dark:text-gray-100"}`}>
                                {message.shared_post.user?.username}
                             </span>
                         </div>
                    
                         {/* Media or Large Text */}
                         {message.shared_post.image_urls?.length > 0 ? (
                           <div className="w-full aspect-square bg-gray-100 dark:bg-slate-900 overflow-hidden">
                             <img src={message.shared_post.image_urls[0]} className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" alt="Post preview" />
                           </div>
                         ) : (
                           <div className={`w-full p-4 min-h-[120px] flex items-center justify-center text-center ${isCurrentUser ? "bg-white/5" : "bg-gray-50 dark:bg-slate-900/50"}`}>
                             <p className={`text-[14px] leading-relaxed line-clamp-4 font-medium ${isCurrentUser ? "text-white" : "text-gray-800 dark:text-gray-200"}`}>
                                {message.shared_post.content || "Shared a post"}
                             </p>
                           </div>
                         )}
                         ...
                         <div className="p-3 pt-2">
                             {message.shared_post.image_urls?.length > 0 && message.shared_post.content && (
                                <p className={`text-[12px] leading-tight line-clamp-2 mb-2 ${isCurrentUser ? "text-white/80" : "text-gray-600 dark:text-gray-400"}`}>
                                    <span className="font-bold mr-1">{message.shared_post.user?.username}</span>
                                    {message.shared_post.content}
                                </p>
                             )}
                             <div className={`text-[11px] font-bold tracking-wide uppercase flex justify-between items-center ${isCurrentUser ? "text-indigo-200" : "text-indigo-500"}`}>
